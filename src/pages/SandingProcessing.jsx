...
const SandingProcessing = () => {
    ...
    const fetchBatches = useCallback(async () => {
        try {
            ...
            const map = {};
            await Promise.all(thicknessBatches.map(async (b) => {
                try {
                    const tRes = await api.get(`/production/thickness/${b.id}`);
                    if (tRes.data && tRes.data.length > 0) {
                        map[b.id] = tRes.data[0]; // most recent first
                    }
                } catch (_) { /* skip */ }
            }));
            setThicknessMap(map);
    ...
    const inputQty = modalBatch && thicknessMap[modalBatch.id]
        ? parseInt(thicknessMap[modalBatch.id].output_quantity, 10)
        : 0;
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(true);
        try {
            await api.post('/production/sanding', {
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Sanding Processing — remove defective boards and prepare for grading</p>
            </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 text-slate-500 uppercase text-xs tracking-wide">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-right">Input Quantity</th>
                                    <th className="px-6 py-3 text-right">Rejected (Thickness)</th>
                                    <th className="px-6 py-3 text-right">Output Quantity</th>
                                    ...
                                    return (
                                        <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-3 text-right text-slate-600">
                                                {tp ? tp.input_quantity : '—'}
                                            </td>
                                            <td className="px-6 py-3 text-right text-red-500">
                                                {tp ? tp.rejected_quantity : '—'}
                                            </td>
                                            <td className="px-6 py-3 text-right font-medium text-emerald-600">
                                                {tp ? tp.output_quantity : '—'}
                                            </td>
            ...
            {modalBatch && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 z-10">
                        <h2 className="text-lg font-semibold text-slate-800 mb-1">Sanding Processing</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                >
                                    {submitting ? 'Processing…' : 'Confirm Sanding'}
                                </button>
                                <button
                                    type="button"
                                    onClick={closeModal}
...
export default SandingProcessing;
