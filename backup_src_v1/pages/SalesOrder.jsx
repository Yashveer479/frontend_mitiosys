        ...
        const fetchData = async () => {
            try {
                ...
                const [cRes, iRes] = await Promise.all([
                    api.get('/customers').catch(() => ({ data: [] })), // Fallback if endpoint missing
                ...
                if (!cRes.data || cRes.data.length === 0) {
                    setCustomers([
                        { _id: '1', name: 'Global Construction Ltd', contact: 'John Doe' },
                        { _id: '2', name: 'Kampala Builders', contact: 'Jane Smith' }
                    ]);
                } else {
                ...
                if (!iRes.data || iRes.data.length === 0) {
                    setInventory([
                        { _id: '1', name: 'MDF Board 18mm', price: 45000 },
                        { _id: '2', name: 'Plywood 12mm', price: 32000 }
                    ]);
                } else {
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="xl:col-span-1">
                        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/40 border border-slate-100 p-8 sticky top-28">
                            ...
                            <button
                                type="submit"
                                disabled={loading || !selectedCustomer || items.some(i => !i.product)}
