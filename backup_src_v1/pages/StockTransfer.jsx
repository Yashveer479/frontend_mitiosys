...
const StockTransfer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Mock API calls - reusing existing endpoints structure
    ...
    const getWarehouseName = (id) => warehouses.find(w => w._id === id)?.name || '...';
    const isFormValid = formData.sourceWarehouse && formData.destWarehouse && formData.product && formData.quantity && formData.transferDate;
                    ...
                    {success && (
                        <div className="absolute top-0 left-0 right-0 bg-emerald-500 text-white py-2 text-center text-xs font-black uppercase tracking-widest animate-fade-in-down z-10">
                            <span className="flex items-center justify-center space-x-2">
                                <CheckCircle size={14} />
                                <span>Transfer Order Created Successfully</span>
                            </span>
                        </div>
                    )}
                                ...
                                <div className="flex-1 w-full space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            {warehouses.filter(w => w._id !== formData.sourceWarehouse).map(w => (
                                                <option key={w._id} value={w._id}>{w.name}</option>
                                            ))}
                                        </select>
