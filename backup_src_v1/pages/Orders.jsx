        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 gap-6">
                {orders.map((order) => (
                    <div
                        key={order._id}
                        className={`bg-white rounded-lg border transition-all duration-300 ${expandedOrder === order._id ? 'border-primary/30 shadow-md ring-2 ring-primary/5' : 'border-slate-200 shadow-sm hover:border-slate-300'
                            }`}
                    >
                        <div
                        ...
                        >
                            <div className="flex items-center space-x-6">
                                <div className={`p-3 rounded-lg transition-colors ${expandedOrder === order._id ? 'bg-primary text-white' : 'bg-slate-50 text-slate-400'
                                    }`}>
                                ...
                                <div>
                                    <div className="flex items-center space-x-3 mb-1">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border ${order.transactionType === 'Direct Export' ? 'bg-primary/5 text-primary border-primary/10' :
                                            order.transactionType === 'Local Sales' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                    ...
                                    <div className="flex items-center space-x-4 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                                        ...
                                        <span>•</span>
                                        <span>{order.items.length} Board Clusters</span>
                                        {order.exportDestination && (
                                            <>
                                                <span>•</span>
                                                <span className="text-primary font-semibold">Transit: {order.exportDestination}</span>
                                            </>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <DocButton
                                                ...
                                                onClick={() => handleGenerateDoc(order._id, 'Proforma Invoice')}
                                            />
                                            <DocButton
                                                title="Order Sheet"
                                                desc="Technical specs"
                                                loading={generating === 'Order Sheet'}
                                                onClick={() => handleGenerateDoc(order._id, 'Order Sheet')}
                                            />
                                            <DocButton
                                                title="Tax Invoice"
                                                desc="URA E-Invoicing"
                                                loading={generating === 'Tax Invoice'}
                                                onClick={() => handleGenerateDoc(order._id, 'Tax Invoice')}
                                            />
                                            <DocButton
                                                title="Delivery Note"
                                                desc="Warehouse release"
                                                loading={generating === 'Delivery Note'}
                                                onClick={() => handleGenerateDoc(order._id, 'Delivery Note')}
                                            />
                                        </div>
                                    </div>
